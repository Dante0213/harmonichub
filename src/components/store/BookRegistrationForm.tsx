...
const bookSchema = z.object({
  name: z.string().min(2, "교재명은 최소 2자 이상이어야 합니다."),
  price: z.string().refine((val) => !isNaN(Number(val)) && Number(val) > 0, {
    message: "유효한 가격을 입력해주세요.",
  }),
  description: z.string().min(10, "교재 설명은 최소 10자 이상이어야 합니다."),
  author: z.string().min(1, "저자를 입력해주세요."),
  publisher: z.string().min(1, "출판사를 입력해주세요."),
  category: z.enum(["피아노", "기타", "바이올린", "드럼", "기타 악기"]),
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>교재명</FormLabel>
              <FormControl>
                <Input placeholder="교재명을 입력하세요" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="출판사명을 입력하세요" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="교재 가격을 입력하세요" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
        ...
        <div className="space-y-2">
          <Label htmlFor="images">교재 이미지 (최대 5장)</Label>
          <Input
            id="images"
            type="file"
            accept="image/*"
