...
export const ProfileActionButtons = ({
  ...
  if (isCurrentUser) {
    return <Button className="w-full h-10" variant="outline" onClick={onEditClick}>
  ...
  return <div className="flex gap-2 flex-wrap">
      <Button onClick={handleFollowToggle} variant={following ? "outline" : "default"} className="flex-1 h-10 mx-0 my-0 px-[5px]">
        {following ? <>
      ...
      {isProfessional && <Button variant={isFavorite ? "outline" : "secondary"} className="flex-1 h-10" onClick={handleFavoriteToggle}>
          <Heart className={`h-4 w-4 mr-2 ${isFavorite ? 'fill-red-500 text-red-500' : ''}`} />
          {isFavorite ? '찜 취소' : '찜하기'}
        </Button>}
      
      <Button variant="outline" onClick={onChatOpen} className="flex-1 h-10 px-[2px]">
